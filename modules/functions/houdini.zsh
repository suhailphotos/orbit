    ...
    python|prefs|use|patch|import|env|doctor)
      ...
      case "$cmd" in
        ...
        use)
          ...
          # if an env exists with a different interpreter, recreate it
          if [[ -x ".venv/bin/python" ]]; then
            local cur_py; cur_py="$(.venv/bin/python -c 'import sys; print(sys.executable)')"
            if [[ "$cur_py" != "$PYBIN" ]]; then
              echo "Recreating .venv with SideFX Python…"
              rm -rf .venv
              uv venv --python "$PYBIN" || return 1
            fi
          else
            uv venv --python "$PYBIN" || return 1
          fi
          ...
          echo "uv venv for $(basename "$proj_root") → $PYBIN"
          ;;
        patch)
          local proj_root="${HOU_PROJECT_ROOT:-$(_hou_project_root)}"
          [[ -n "$proj_root" ]] || { echo "Not inside a project (pyproject.toml not found)."; return 1; }
          local pref; pref="$(_hou_pref_dir_for_version "$ver")"
          export HOUDINI_USER_PREF_DIR="$pref"; mkdir -p "$pref"
          local site; site="$(_hou_site_packages_for_venv "$proj_root/.venv")" || { echo "No .venv yet; run 'hou use' or 'uv venv' first."; return 1; }
...
Usage:
  ...
  hou use     [VER|latest]                # create/recreate .venv with SideFX python
